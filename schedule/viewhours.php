<title>
 <?php echo $_POST['employeeid']?> - <?php echo $_POST['agentname']?> <?php echo $_POST['agenltname']?>'s Working Hours 
</title>
<?php
...
//if time out is greater than midnight
			...
			$tomtimeout=$endworkingtime;
				...
				if($strtodaymidnight<=$strtime_in && $strtodaynightdiffend>=$strtime_in){
					...
					//$endworkingtime=$strtime_out;
					//$startworkingtime=$strtime_in;
			//	echo $dates."-naa today ".$strtime_out."<br> reymond";
                                    
					$todayworkinghourswithoutnightdiff=(($endworkingtime-$startworkingtime)/3600);
                                        
//echo $todayworkinghourswithoutnightdiff."<br> ";
                                    
					if(date('h',$endworkingtime) == '11'){
                                            $todaynightdiffhours = '1';
                                            $todayworkinghourswithoutnightdiff = '8';
                                        }
							if($endworkingtime>$strnightdiff_out){
	 ...
	 if($rowtimelogs['status']=='6th day OT'){
			...
			<tr <?php if($rowtimelogs['status']=='Vacation Leave' || $rowtimelogs['status']=='Wedding Leave' || $rowtimelogs['status']=='6th day OT' || $rowtimelogs['status']=='HOLI'){ echo "bgcolor='yellowgreen'";}?>  onMouseOver="this.className='highlight'" onMouseOut="this.className='normal'">
				...
				//echo $todaynightdiffhours."+".$tomnightdiffhours."-".$late_hours." = <br>";
						...
						if($hourswithnightdiff>8)
						{
						$hourswithnightdiff=8;
						}
...
FROM `finalhourstable` where shiftdate='".$dates."' and userid='".$userid."'");
				...
				?>
                <input type="text" name="approveearly[]" style="font-size:11px; background-color:#B0F9A6" size="8" value="<?php if($earlyz!='' || $earlyz!='0'){ echo $earlyz; } else { echo "0";}?>" /></td>
			  ...
			  ?>
			  <?php echo round($late_hours,2)?>
              
              </td>
                <td <?php if($undertimehour>0){ echo "bgcolor='orange'";} ?>><?php 
				...
				$undertimehour=9-$hourhour-$late_hours;
				if($approve=='1')
				{
				...
				?></td>
              <td><?php if($rowtimelogs['status']=='6th day OT'){
					//$Regulat_OT=0;
					}	echo round($Regulat_OT,2); ?></td>
                <td ><input type="text" name="approve[]" style="font-size:11px; background-color:#B0F9A6" size="8"  value="<?php if($regular_ot_hoursz!='' || $regular_ot_hoursz!='0'){ 
			  if($rowtimelogs['status']=='6th day OT'){
					$regular_ot_hoursz=0;
					}	
      ...
      <td><?php if($rowtimelogs['status']=='6th day OT'){
                ...
                <td><input type="text" name="approvewithnd[]" style="font-size:11px; background-color:#B0F9A6" size="8" value="<?php if($nightdiff_ot_hoursz!='' || $nightdiff_ot_hoursz!='0'){ echo $nightdiff_ot_hoursz; } else { echo "0";}?>" /></td>
             
            <!--  <td><?php echo $rowtimelogs['thisdaysshift']?></td>
                <td><?php echo strtotime($skedin)?></td>
                <td><?php echo strtotime($rowtimelogs['startshift'])?></td>
                <td><?php echo $skedout?></td>
                <td><?php echo $rowtimelogs['endshift']?></td>
                <td><?php echo $strtime_in?></td><td><?php echo $strsched_in?></td>!-->
            </tr>
...
<input type="hidden" value="<?php echo $_POST['agentname']?>" name="agentname" />
<input type="hidden" value="<?php echo $_POST['agenltname']?>" name="agenltname" />
  
<input type="hidden" value="<?php echo $pyear?>-<?php echo $pmonth?>-<?php echo $ppayperiod?>" name="payperiod" />

  
<input type="submit" name="submit" value="Approve" <?php if($approve=='1'){ echo'disabled="disabled"';} ?> onclick="return confirm('Are you Sure?');" />
</td></tr>
</table>
</form>
...
$.fn.editInPlace.defaults = {
	...
	default_text:		"(Click here to change value)", // string: text to show up if the element that has this functionality is empty
...
$.extend(InlineEditor.prototype, {
	...
	replaceContentWithEditor: function() {
		...
		this.dom.html('<form class="inplace_form" style="display: inline; margin: 0; padding: 0;" autocomplete="off"></form>')
			.find('form')
	...
	createEditorElement: function() {
		...
		if ("select" === this.settings.field_type)
			...
			editor = $('<input type="text" ' + this.inputNameAndClass() 
				+ ' size="' + this.settings.text_size  + '" id="'+this.dom.attr("id")+'_shit" /><script>$("#'+this.dom.attr("id")+'_shit").AnyTime_picker({  format: "%Y-%m-%d %H:%i:%s",baseYear: 2011,earliest: new Date(2011,1,1) } )</script>');

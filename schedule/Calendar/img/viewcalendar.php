...
<script type="text/javascript">
	...
	function startCalendar(month, year) {
		...
		new Ajax.Updater('calendarFooter', 'calfooter.php', {method: 'post', postBody: 'action=startCalendar&month='+month+'&year='+year+''});
	}
	...
	function addEvent(day, month, year, eventtitle, body) {
		if(day && month && year && body) {
			// alert('Add Event\nDay: '+day+'\nMonth: '+month+'\nYear: '+year+'\nBody: '+body);
			new Ajax.Request('rpc.php', {method: 'post', postBody: 'action=addEvent&d='+day+'&m='+month+'&y='+year+'&body='+body+'&eventtitle='+eventtitle+'', onSuccess: highlightEvent(day)});
			$('evtBody').value = '';
			$('timesched').value = '';
		} else {
...
<!-- 		</div> -->
		...
		<div style="display: none; margin-top: 10px;" id="addEventForm">
			...
			<br>
			<input type="button" value="Add Event" onClick="addEvent($F('evtDay'), $F('evtMonth'), $F('evtYear'), $F('eventtitle'), $F('evtBody'));">

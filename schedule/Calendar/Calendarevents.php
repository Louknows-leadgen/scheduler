...
$result = mysql_query("SELECT DATE_FORMAT(eventDate,'%d') AS day,eventTitle,employeeid,id,employeename,eventContent,leavecredit,addedby FROM eventcal WHERE eventDate BETWEEN  '$current_year/$current_month/01' AND '$current_year/$current_month/$total_days_of_current_month'");
$xs=0;
while($row_event = mysql_fetch_object($result))
{
	...
	$events[intval($row_event->day)] .= '<li><span class="title">'.stripslashes($row_event->eventTitle).' - '.stripslashes($row_event->leavecredit).' hrs <br>'.stripslashes($row_event->employeeid).' - '.stripslashes($row_event->employeename).'</span><span class="desc">'.$contentevent.'</span><br>';
				...
				for($j=0; $j<7;$j++)
				{
					...
					if($day>0 && $day<=$total_days_of_current_month)
					{
						...
						?>
                         <script type="text/javascript">
                        jQuery(document).ready(function($) {
                          $('#faceboxni<?=$current_year.$current_month.$day?>').facebox({
                            loadingImage : 'facebox/loading.gif',
                            closeImage   : 'facebox/closelabel.png',
                            div: '#box<?=$current_year.$current_month.$day?>' 
                          })
                        })
                      </script>
                        <td  <?
						
						//check if the date is today id="faceboxni<?=$current_year.$current_month.$day"
						if($date_form == $today)
						{
							echo ' class="today"';
						}
						...
						?><br>
                       <a href="#box<?=$current_year.$current_month.$day?>" rel="facebox">add</a>
              		  
                      <div id="box<?=$current_year.$current_month.$day?>" style="display: none;">                      
                      <form method="get" action="#">
                      <table>
                      <tr>
                      <td> Date : </td><td><? echo $current_year.'/'.$current_month.'/'.$day;?></td></tr>

		...
		while($rowlog = mysqli_fetch_array($resultlog))
		   {  
			     ...
			     if($check=='1')
			       {
					  ...
					  if($holidayhours >= 5)
					   ...
					   else if($checktype['holidaytype'] == 'Double Holiday')
					   {
						   $dholidayamt = $dholidayamt + ($hourlyrate * $holidayhours * ($multiplier[50] - 1));
						   $dholidayhours = $dholidayhours + $holidayhours;
					   }
					   
					   echo "<td>".$checktype['holidaytype']."(".$holidayhours." hrs)</td>";
				   }
				   $totalholidayamt = $rholidayamt + $sholidayamt + $dholidayamt;
			  ...
			  else
			  {
			   ...
			   /*Not Present*/
			   $absentlog = mysqli_query($con,"SELECT * FROM finalhourstable WHERE shiftdate >= '$paystart' AND shiftdate <= '$payend' AND userid = '".$rowgetemplyees['employeeid']."' AND (status = 'Absent' OR status = 'USUP' OR status = 'LMAT' OR status = 'LOAM' OR status = 'BVUNP' OR status = 'LOAU' OR status = 'NCNS' OR status = 'SKUNP') GROUP BY shiftdate");
...
Proration = <?php  echo $proration;?><br />
...
Total ND Amt = <?php echo $nightsecs;?> * hourlyrate(<?php echo $hourlyrate;?>) * <?php echo $rowgetemplyees['nightrate'];?><br />
...
Double Holiday Pay = totalhours * hourlyrate(<?php echo $hourlyrate;?>) * Regular Holiday Rate(x2) <br />
Double Holiday Pay = <?php echo $dholidayhours;?> * hourlyrate(<?php echo $hourlyrate;?>) * 2 <br />
Double Holiday Pay = <?php echo $dholidayamt;?><br /><br />	
	
Regular Holiday Pay = totalhours * hourlyrate(<?php echo $hourlyrate;?>) * Regular Holiday Rate(x1) <br />
...
Special Holiday Pay = totalhours * hourlyrate(<?php echo $hourlyrate;?>) * Special Holiday Rate(x0.35) <br />
...
<?php
		   ...
		   /*Summary Amount*/
           if($rowgetemplyees['nightrate'] == '0')//for exempt = 0 no nyt dif and overtime
		   {
		      $nightamt = 0;
...
<?php
	...
	$nontaxableotherincome = 0;
	 ...
	 while($resultadjustdata = mysqli_fetch_array($resultadjustment))
	 {
		...
		{
			$nontaxableotherincome = $nontaxableotherincome + $resultadjustdata['amount'];
		   ...
		   { 
			...
			if($rowgetemplyees['costcenterid'] != 'FLT')
		    {
               ...
               $rowmonthsummary=mysqli_fetch_array($monthsummary);
				...
				/* $grossmonth = $grossmonthprevious + $rowgetsummary['basicpay'] + $rowgetsummary['holidaypay'] + $rowgetsummary['regot'] + $rowgetsummary['6thdayot'] + $rowgetsummary['7thdayot'] + $rowgetsummary['totalnd'] + $rowgetsummary['taxableotherinc'] + $rowgetsummary['nontaxable'] - $rowgetsummary['deductions']; */
				$grossmonth = $rowgetsummary['basicpay']*2; /* Basis Basic pay only */
		    echo "Month Gross Income = ".$grossmonth;
			...
			$value = mysqli_fetch_array($deduct);
			if( $rowgetemplyees['periodrate'] <= 10000){
				$cphic = $phic = (275 /2) / 2;
			}else if( $rowgetemplyees['periodrate'] > 40000){
				$cphic = $phic = (1100 /2) /2 ;
			}else{
				$cphic = $phic = ((  $rowgetemplyees['periodrate'] * 0.0275 ) /2) /2 ;
			}
...
?>
<tr>
<td align = "center">SSS(based on Month gross income)</td>
<td>&nbsp;</td>
<td align = "center"><? echo $sss;?></td>
</tr>
<tr>
<td align = "center">PHIC(based on Month gross income)</td>
<td>&nbsp;</td>
...
<table align ="center" width="600">
  <?php
		...
		$totaltax=0;
	    echo "<tr><td>SemiMonthly Taxable Income above Php 10,000.00 at 25%<br></td></tr>";
	
		echo "<tr><td>Total Taxable Income : ".$nettaxable."</td></tr>";
	    
	     if($nettaxable > '10000')
		 {
			 $excess = $nettaxable - 10000;
			 echo "<tr><td>Amount Excess  = ".$excess." * 25% <br></td></tr>";
			 $totaltax = $excess*0.25;
		     echo "<tr><td>Tax = ".$totaltax."<br></td></tr>";
		 }
	     else 
		 {
			 echo "<tr><td>Amount Excess  = 0 * 25% <br></td></tr>";
		     echo "<tr><td>Tax = <br></td></tr>";
...
<p>&nbsp;</p>
<table border = "1" align ="center" width="450">
<td align = "center" colspan = "4">Non Taxable Income</td></tr>
<tr><td width="50" align = "center"> Allowance ID </td>
	<td width="200" align = "center"> Definition </td> 
	<td width="78" align = "center"> Debit </td>
...
<?php
	    ...
	    /*Deleting Non Taxable Income Logs*/
	       
	      mysqli_query($con,"DELETE FROM prlotherinclog WHERE employeeid = '".$rowgetemplyees['employeeid']."' AND payperiod = '".$_GET['payperiod']."' ");
	     
	     /*Displaying NonTaxable Incomes and Saving Updated Nontaxable Incomes in prlotherinclog*/
		...
		while($otherincdata = mysqli_fetch_array($otherinc))
		   {  
			 ...
			 $otherincamt =mysqli_query($con,"SELECT othincvalue, othincid, taxable from prlothinctable WHERE othincid = '$otherincdata[othincid]'");
			  ...
			  echo "<tr><td align = 'center'>".$otherincdata['othincid']."</td><td align = 'center'>".$otherincdata['othincdescription']."</td><td align = 'center'>".$halfamt."</td><td>&nbsp;</td></tr>";
              $nontaxableincome = $nontaxableincome + $halfamt;
				 
			  $sqlothinclog="INSERT INTO prlotherinclog (employeeid, payperiod, description	, amount)
                VALUES
                   ('".$rowgetemplyees['employeeid']."','".$_GET['payperiod']."','".$otherincdata['othincdescription']."', '".$halfamt."' )";	
	
	          mysqli_query($con,$sqlothinclog);
			 ...
			 $otherincamt =mysqli_query($con,"SELECT othincvalue, othincid, taxable from prlothinctable WHERE othincid = '$otherincdata[othincid]'");
              ...
              $nontaxableincome = $nontaxableincome + $amtnontax['othincvalue']*$proration;
			  }
		   }	
		   
		   echo "<tr><td align = 'center' colspan = '2'> Total </td><td align = 'center' colspan = '2'>".$nontaxableincome."</td></tr>";
...
<?php
      /*Loan Deductions*/
		...
		while($loandata = mysqli_fetch_array($loan))
		   ...
		   /*SSS & HDMF Loans*/	
            $loan =mysqli_query($con,"SELECT * from prlloansgov WHERE empid = '".$rowgetemplyees['employeeid']."' AND (deductiondate = '".$paydate."' || deductiondate = '1025')");
		while($loandata = mysqli_fetch_array($loan))
		   { 
			...
			if ($loandata['deductiondate']=='1025')
			 ...
			 echo "<td align='center'>"; echo $loanamount; echo "</td>"; 
			 echo "<td align='center'> N/A </td>";			 
			 echo "</tr>";
...
<p>&nbsp;</p>
<p>Total Net Income: Taxable Income + Non Taxable Income + NET Manual Adjustment - Total Tax - Total Loan Deductions</p>
<p>Total Net Income: <?php echo $nettaxable;?> + <?php echo $nontaxableincome;?> + <?php echo $nontaxableotherincome;?> - <?php echo $totaltax;?> - <?php echo $loanamt;?></p>
<?php

		  
		   $netincome = $nettaxable + $nontaxableincome + $nontaxableotherincome - $totaltax;
	 ...
	 mysqli_query($con,$loanlog);	   
		...
		while($loandata = mysqli_fetch_array($loan))
		   {  
			 ...
			 $loanamt = $loanamt + $loanamount;	
			
 	         $loanlog="INSERT INTO prlloansgovlog (user, ipaddress, description, payperiod,employeeid,loantype,loanid)
               VALUES
                  ('".$_SESSION['Username']."','".$_SERVER['REMOTE_ADDR']."', '".$loanamt."','".$_POST['payperiod']."','".$rowgetemplyees['employeeid']."','".$loandata['loantype']."','".$loandata['loanid']."')";	
	          mysqli_query($con,$loanlog);	   
...
/*END OF LOAN PROBLEM*/				
		...
		totalnd = '$nightamt',nontaxable = '$nontaxableincome', deductions = '$deductions', nonttaxableotherinc = '$nontaxableotherincome', loandeductions = '$loanamt', 
